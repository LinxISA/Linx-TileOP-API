...
// TSRL performs a logical (unsigned) right shift on the host reference path:
// src0 is converted to its unsigned counterpart before shifting so the sign
// bit does not propagate, matching the hardware v.srl semantics.
  ...
  for (size_t i = 0; i < tile_shape::ValidRow; ++i)
    for (size_t j = 0; j < tile_shape::ValidCol; ++j) {
      ...
      dst[idx] = static_cast<typename tile_shape::DType>(
          static_cast<UDType>(src0[idx]) >> src1[idx]);
    }
}
...
void TSRL_Impl(tile_shape &dst, tile_shape &src0, tile_shape &src1) {
  ...
  if constexpr (std::is_same<typename tile_shape::DType, int64_t>::value ||
    ...
    if constexpr (tile_shape::isRowMajor) {
      TSrl_RowMajor_Imp<tile_shape>(dst.data(), src0.data(), src1.data());
    } else {
      TSrl_ColMajor_Imp<tile_shape>(dst.data(), src0.data(), src1.data());
    }
  } else {
