...
`TLOAD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TLOAD` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| Function | `0` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:1774` |
...
`TLOAD` 的 row stride in **bytes**，并在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TLOAD`
...
| --- | --- | ---: | ---: | ---: |
| `TLOAD` | `TEPL` | `0` (`0x00`) | `0` | `2` |

`TLOAD` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.TLSU TLOAD, DataType
