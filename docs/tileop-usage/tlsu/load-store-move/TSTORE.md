...
`TSTORE` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSTORE` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| Function | `1` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:1905` |
...
`TSTORE` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSTORE`
...
| --- | --- | ---: | ---: | ---: |
| `TSTORE` | `TEPL` | `1` (`0x01`) | `1` | `2` |

`TSTORE` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.TLSU TSTORE, DataType
