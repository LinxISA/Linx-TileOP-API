...
`TMOV` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMOV` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| Function | `2` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:207` |
...
`TMOV` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMOV`
...
| --- | --- | ---: | ---: | ---: |
| `TMOV` | `TEPL` | `2` (`0x02`) | `2` | `2` |

`TMOV` 是 selector-encoded block operation，不是独立 standalone opcode。
