...
`TPREFETCH` is a TileOP C++ interface for the PTO ISA v0.58.3 `TPREFETCH` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `tlsu` |
| 编码 carrier | `TEPL` |
| Logical selector | `3` (`0x03`) |
| Function | `3` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:2171` |
...
`TPREFETCH` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TPREFETCH`
...
| --- | --- | ---: | ---: | ---: |
| `TPREFETCH` | `TEPL` | `3` (`0x03`) | `3` | `2` |

`TPREFETCH` 是 selector-encoded block operation，不是独立 standalone opcode。
