...
`MSCATTER_ADD` 是由 TLSU 执行的选择器编码 Tile 操作：它使用 index 和 value Tile，按逻辑线性元素下标对 GM 元素执行原子归约加（reduction add）；该操作没有目标 Tile，也不发布旧值。其当前指令 contract 规定了确切的 bundle 形式和发布边界。
...
索引 Tile 支持整数类型；归约传输数据 Tile 支持 FP16、BF16、FP32、FP64、S32、U32、U64（对应 `GMReductionOperationDataTypeLegal`）。

| 操作数角色 | 类型要求 |
| --- | --- |
| 数据 Tile | `value` 必须使用一种 FP16、BF16、FP32、FP64、S32、U32、U64 类型。 |
...
| 项目 | 规则 |
| --- | --- |
| 有效元素 | 逐元素操作通常仅对输入共同的有效区域定义结果；未明确规定的 padding 不应读取或依赖。 |
...
## 异常和边界行为
    ...
    类型不匹配、非法形状或布局、未初始化的输入、非法 PE mask、错误的 Tile 位置或不合法的属性组合，会在编译期或执行前检查阶段被拒绝。`PE_MASK=0000` 时操作不产生状态或内存影响；非法调用不会发布部分副作用。padding、alias、NaN/无穷值及 fault 行为以本页已经列出的约束和边界说明为准，未明确声明的状态不可依赖。

## 结果说明

    成功调用后，`MSCATTER_ADD` 原子地把每个有效位置的 value 归约进对应 GM 元素；重复地址的合并顺序由实现定义。若操作的约束或参数说明另有规定，以对应说明为准。
...
void atomic_reduce(ElementIndices &element_indices, Transfer &value) {
  ...
  MSCATTER_ADD(0x1000ull, element_indices, value, 256, 2);
}
```
