...
`MSCATTER_MIN` 是由 TLSU 执行的选择器编码 Tile 操作：它使用 index 和 value Tile，按逻辑线性元素下标对 GM 元素执行原子归约取最小值（reduction min）；该操作没有目标 Tile，也不发布旧值。其当前指令 contract 规定了确切的 bundle 形式和发布边界。
...
索引 Tile 支持整数类型；归约传输数据 Tile 支持 S32、S64、U32、U64（对应 `GMReductionOperationDataTypeLegal`）。

| 操作数角色 | 类型要求 |
| --- | --- |
| 数据 Tile | `value` 必须使用一种 S32、S64、U32、U64 类型。 |
...
## 结果说明
    ...
    成功调用后，`MSCATTER_MIN` 原子地把每个有效位置的 value 归约进对应 GM 元素；重复地址的合并顺序由实现定义。若操作的约束或参数说明另有规定，以对应说明为准。
...
void atomic_reduce(ElementIndices &element_indices, Transfer &value) {
  ...
  MSCATTER_MIN(0x1000ull, element_indices, value, 256, 2);
}
```
