...
`MGATHER_XOR` 是由 TLSU 执行的选择器编码 Tile 操作：它使用 index 和 value Tile，按逻辑线性元素下标对 GM 元素执行原子按位异或（atomic xor），并把运算前观察到的旧值发布到目标 Tile；其当前指令 contract 规定了确切的 bundle 形式和发布边界。
...
| 操作数角色 | 类型要求 |
| --- | --- |
| 数据 Tile | `observedOld`、`value` 必须使用同一种 U32、U64 类型。 |
...
## 结果说明
    ...
    成功调用后，`MGATHER_XOR` 原子地更新每个有效位置的 GM 元素，并把运算前读到的旧值写入输出 Tile 的有效区域；输出 padding 和未明确声明的副作用不可依赖。若操作的约束或参数说明另有规定，以对应说明为准。
...
void atomic_rmw(Transfer &observed_old, ElementIndices &element_indices,
  ...
  MGATHER_XOR(observed_old, 0x1000ull, element_indices, value, 256, 2);
}
```
