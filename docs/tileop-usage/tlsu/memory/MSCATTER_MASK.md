...
`MSCATTER_MASK` is a TileOP C++ interface for the PTO ISA v0.58.3 `MSCATTER_MASK` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `MSCATTER_MASK` |
| ISA 操作 | `MSCATTER_MASK` |
| 执行引擎 | `TLSU` |
| 指令分类 | `tlsu` |
| 编码 carrier | `TEPL` |
| Logical selector | `7` (`0x07`) |
| Function | `7` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:548` |
...
`MSCATTER_MASK` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `MSCATTER_MASK`
...
| --- | --- | ---: | ---: | ---: |
| `MSCATTER_MASK` | `TEPL` | `7` (`0x07`) | `7` | `2` |

`MSCATTER_MASK` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.TLSU MSCATTER_MASK, DataType
