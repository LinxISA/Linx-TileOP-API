...
`MGATHER_MASK` 是由 TLSU 执行的选择器编码 Tile 操作：它只收集谓词恰为 1 的 lane，并用选定的 padding 值填充禁用的 lane；其当前指令 contract 规定了确切的 bundle 形式和发布边界。
...
template <
    ...
    typename tile_shape_mask,
    typename gm_shape,
    TmaPadValue Pad = TmaPadValue::Null>
inline void MGATHER_MASK(
    tile_shape_out &dst,
    const gm_shape &src,
...
## 结果说明
    ...
    成功调用后，`MGATHER_MASK` 更新输出 Tile 的有效区域；输入 Tile 通常保持不变，输出 padding 和未明确声明的副作用不可依赖。若操作的约束或参数说明另有规定，以对应说明为准。
...
void gather_masked(float *out, const float *base, const uint32_t *offsets,
  ...
  MGATHER_MASK<Values, ElementIndices, Mask, GM, TmaPadValue::Zero>(
