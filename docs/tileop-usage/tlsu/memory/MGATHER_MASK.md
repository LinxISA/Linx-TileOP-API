...
`MGATHER_MASK` is a TileOP C++ interface for the PTO ISA v0.58.3 `MGATHER_MASK` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| Function | `6` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:516` |
...
`MGATHER_MASK` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `MGATHER_MASK`
...
| --- | --- | ---: | ---: | ---: |
| `MGATHER_MASK` | `TEPL` | `6` (`0x06`) | `6` | `2` |

`MGATHER_MASK` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.TLSU MGATHER_MASK, DataType
