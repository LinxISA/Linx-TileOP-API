...
`MSCATTER_MAX` 是由 TLSU 执行的选择器编码 Tile 操作：它使用 index 和 value Tile，按逻辑线性元素下标对 GM 元素执行原子归约取最大值（reduction max）；该操作没有目标 Tile，也不发布旧值。其当前指令 contract 规定了确切的 bundle 形式和发布边界。
...
## 结果说明
    ...
    成功调用后，`MSCATTER_MAX` 原子地把每个有效位置的 value 归约进对应 GM 元素；重复地址的合并顺序由实现定义。若操作的约束或参数说明另有规定，以对应说明为准。
...
void atomic_reduce(ElementIndices &element_indices, Transfer &value) {
  ...
  MSCATTER_MAX(0x1000ull, element_indices, value, 256, 2);
}
```
