...
`MGATHER` is a TileOP C++ interface for the PTO ISA v0.58.3 `MGATHER` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| Function | `4` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:461` |
...
`MGATHER` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `MGATHER`
...
| --- | --- | ---: | ---: | ---: |
| `MGATHER` | `TEPL` | `4` (`0x04`) | `4` | `2` |

`MGATHER` 是 selector-encoded block operation，不是独立 standalone opcode。
