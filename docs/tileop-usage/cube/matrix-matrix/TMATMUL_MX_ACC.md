...
`TMATMUL_MX_ACC` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMATMUL_MX_ACC` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TMATMUL_MX_ACC` |
| ISA 操作 | `TMATMUL_MX_ACC` |
| 执行引擎 | `CUBE` |
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `6` (`0x06`) |
| Function | `6` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5441` |

## C++ 接口

```cpp
TMATMUL_MX_ACC(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TMATMUL_MX_ACC` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMATMUL_MX_ACC`
...
| --- | --- | ---: | ---: | ---: |
| `TMATMUL_MX_ACC` | `TEPL` | `6` (`0x06`) | `6` | `4` |

`TMATMUL_MX_ACC` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TMATMUL_MX_ACC, DataType
