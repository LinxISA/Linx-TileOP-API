...
`TMATMUL_ACC` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMATMUL_ACC` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `2` (`0x02`) |
| Function | `2` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5023` |

## C++ 接口

```cpp
TMATMUL_ACC(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TMATMUL_ACC` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMATMUL_ACC`
...
| --- | --- | ---: | ---: | ---: |
| `TMATMUL_ACC` | `TEPL` | `2` (`0x02`) | `2` | `4` |

`TMATMUL_ACC` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TMATMUL_ACC, DataType
