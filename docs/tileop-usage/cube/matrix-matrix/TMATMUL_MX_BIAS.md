...
`TMATMUL_MX_BIAS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMATMUL_MX_BIAS` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TMATMUL_MX_BIAS` |
| ISA 操作 | `TMATMUL_MX_BIAS` |
| 执行引擎 | `CUBE` |
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `5` (`0x05`) |
| Function | `5` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5521` |

## C++ 接口

```cpp
TMATMUL_MX_BIAS(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TMATMUL_MX_BIAS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMATMUL_MX_BIAS`
...
| --- | --- | ---: | ---: | ---: |
| `TMATMUL_MX_BIAS` | `TEPL` | `5` (`0x05`) | `5` | `4` |

`TMATMUL_MX_BIAS` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TMATMUL_MX_BIAS, DataType
