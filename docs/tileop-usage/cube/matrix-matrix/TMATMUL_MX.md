...
`TMATMUL_MX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMATMUL_MX` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `4` (`0x04`) |
| Function | `4` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5372` |

## C++ 接口

```cpp
TMATMUL_MX(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TMATMUL_MX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMATMUL_MX`
...
| --- | --- | ---: | ---: | ---: |
| `TMATMUL_MX` | `TEPL` | `4` (`0x04`) | `4` | `4` |

`TMATMUL_MX` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TMATMUL_MX, DataType
