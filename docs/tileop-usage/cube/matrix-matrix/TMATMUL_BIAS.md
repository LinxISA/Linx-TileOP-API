...
`TMATMUL_BIAS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMATMUL_BIAS` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `1` (`0x01`) |
| Function | `1` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5303` |

## C++ 接口

```cpp
TMATMUL_BIAS(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TMATMUL_BIAS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMATMUL_BIAS`
...
| --- | --- | ---: | ---: | ---: |
| `TMATMUL_BIAS` | `TEPL` | `1` (`0x01`) | `1` | `4` |

`TMATMUL_BIAS` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TMATMUL_BIAS, DataType
