...
| 参数 | 默认值 |
| --- | --- |
| `tile_role_v<Mtx>` | `= Location::Right && tile_role_v<Vec> == Location::Left) PTO_SHARED_INLINE void TGEMV_MX_BIAS(D &d` |
...
## 结果说明
    ...
    成功调用后，`TGEMV_MX_BIAS` 更新输出 Tile 的有效区域；输入 Tile 通常保持不变，输出 padding 和未明确声明的副作用不可依赖。若操作的约束或参数说明另有规定，以对应说明为准。
