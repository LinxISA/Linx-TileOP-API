...
`TGEMV_MX_BIAS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TGEMV_MX_BIAS` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TGEMV_MX_BIAS` |
| ISA 操作 | `TGEMV_MX_BIAS` |
| 执行引擎 | `CUBE` |
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `21` (`0x15`) |
| Function | `21` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6031` |

## C++ 接口

```cpp
TGEMV_MX_BIAS(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TGEMV_MX_BIAS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TGEMV_MX_BIAS`
...
| --- | --- | ---: | ---: | ---: |
| `TGEMV_MX_BIAS` | `TEPL` | `21` (`0x15`) | `21` | `4` |

`TGEMV_MX_BIAS` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TGEMV_MX_BIAS, DataType
