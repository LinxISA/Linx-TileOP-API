...
`TGEMV_BIAS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TGEMV_BIAS` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `17` (`0x11`) |
| Function | `17` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5818` |

## C++ 接口

```cpp
TGEMV_BIAS(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TGEMV_BIAS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TGEMV_BIAS`
...
| --- | --- | ---: | ---: | ---: |
| `TGEMV_BIAS` | `TEPL` | `17` (`0x11`) | `17` | `4` |

`TGEMV_BIAS` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TGEMV_BIAS, DataType
