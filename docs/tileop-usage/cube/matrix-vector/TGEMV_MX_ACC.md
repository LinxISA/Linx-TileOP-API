...
```cpp
PTO_SHARED_INLINE void TGEMV_MX_ACC(
    tile_shape_d &d,
    tile_shape_c &c,
    tile_shape_mtx &mtx,
...
PTO_SHARED_INLINE void TGEMV_MX_ACC(D &d, C &c, Mtx &mtx, Vec &vec, ScaleVec &scale_vec);
PTO_SHARED_INLINE void TGEMV_MX_ACC(D &d, C &c, Mtx &mtx, ScaleMtx &scale_mtx, Vec &vec);
```
...
成功调用后，`TGEMV_MX_ACC` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
using Acc = CubeAccumulatorM16<float, 1, 16>;
...
void gemv_mx_acc(Acc &d, Acc &c, Matrix &matrix, ScaleMatrix &scale_matrix,
                 Vec &vec, ScaleVec &scale_vec) {
  TGEMV_MX_ACC(d, c, matrix, scale_matrix, vec, scale_vec);
}
```
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TGEMV_MX_ACC.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/matrix-and-matrix-vector/matrix-vector/TGEMV_MX_ACC.md)。
