...
`TGEMV_MX_ACC` is a TileOP C++ interface for the PTO ISA v0.58.3 `TGEMV_MX_ACC` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `22` (`0x16`) |
| Function | `22` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6106` |

## C++ 接口

```cpp
TGEMV_MX_ACC(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TGEMV_MX_ACC` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TGEMV_MX_ACC`
...
| --- | --- | ---: | ---: | ---: |
| `TGEMV_MX_ACC` | `TEPL` | `22` (`0x16`) | `22` | `4` |

`TGEMV_MX_ACC` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TGEMV_MX_ACC, DataType
