...
`TGEMV_MX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TGEMV_MX` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `20` (`0x14`) |
| Function | `20` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5959` |

## C++ 接口

```cpp
TGEMV_MX(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TGEMV_MX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TGEMV_MX`
...
| --- | --- | ---: | ---: | ---: |
| `TGEMV_MX` | `TEPL` | `20` (`0x14`) | `20` | `4` |

`TGEMV_MX` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TGEMV_MX, DataType
