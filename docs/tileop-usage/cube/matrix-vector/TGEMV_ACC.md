...
`TGEMV_ACC` is a TileOP C++ interface for the PTO ISA v0.58.3 `TGEMV_ACC` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 指令分类 | `cube` |
| 编码 carrier | `TEPL` |
| Logical selector | `18` (`0x12`) |
| Function | `18` |
| Mode | `4` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:5889` |

## C++ 接口

```cpp
TGEMV_ACC(...);  // see matrix-postprocess.md for the complete options overload set
```
...
`TGEMV_ACC` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TGEMV_ACC`
...
| --- | --- | ---: | ---: | ---: |
| `TGEMV_ACC` | `TEPL` | `18` (`0x12`) | `18` | `4` |

`TGEMV_ACC` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.CUBE TGEMV_ACC, DataType
