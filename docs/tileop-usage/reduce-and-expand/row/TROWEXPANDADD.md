...
`TROWEXPANDADD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDADD` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDADD` |
| ISA 操作 | `TROWEXPANDADD` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `69` (`0x45`) |
| Function | `5` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8491` |
...
`TROWEXPANDADD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDADD`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDADD` | `TEPL` | `69` (`0x45`) | `5` | `2` |

`TROWEXPANDADD` 是 selector-encoded block operation，不是独立 standalone opcode。
