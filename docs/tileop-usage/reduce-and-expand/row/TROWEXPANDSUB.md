...
`TROWEXPANDSUB` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDSUB` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDSUB` |
| ISA 操作 | `TROWEXPANDSUB` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `70` (`0x46`) |
| Function | `6` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8521` |
...
`TROWEXPANDSUB` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDSUB`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDSUB` | `TEPL` | `70` (`0x46`) | `6` | `2` |

`TROWEXPANDSUB` 是 selector-encoded block operation，不是独立 standalone opcode。
