...
`TROWEXPANDMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDMIN` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDMIN` |
| ISA 操作 | `TROWEXPANDMIN` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `74` (`0x4a`) |
| Function | `10` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8643` |
...
`TROWEXPANDMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDMIN` | `TEPL` | `74` (`0x4a`) | `10` | `2` |

`TROWEXPANDMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
