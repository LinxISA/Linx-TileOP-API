...
`TROWMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWMIN` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `66` (`0x42`) |
| Function | `2` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8248` |
...
`TROWMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TROWMIN` | `TEPL` | `66` (`0x42`) | `2` | `2` |

`TROWMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
