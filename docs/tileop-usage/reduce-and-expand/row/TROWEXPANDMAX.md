...
`TROWEXPANDMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDMAX` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDMAX` |
| ISA 操作 | `TROWEXPANDMAX` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `73` (`0x49`) |
| Function | `9` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8613` |
...
`TROWEXPANDMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDMAX` | `TEPL` | `73` (`0x49`) | `9` | `2` |

`TROWEXPANDMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
