...
`TROWSUM` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWSUM` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `64` (`0x40`) |
| Function | `0` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8206` |
...
`TROWSUM` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWSUM`
...
| --- | --- | ---: | ---: | ---: |
| `TROWSUM` | `TEPL` | `64` (`0x40`) | `0` | `2` |

`TROWSUM` 是 selector-encoded block operation，不是独立 standalone opcode。
