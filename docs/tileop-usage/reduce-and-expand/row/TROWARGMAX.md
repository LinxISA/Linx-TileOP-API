...
`TROWARGMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWARGMAX` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `76` (`0x4c`) |
| Function | `12` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8308` |
...
`TROWARGMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWARGMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TROWARGMAX` | `TEPL` | `76` (`0x4c`) | `12` | `2` |

`TROWARGMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
