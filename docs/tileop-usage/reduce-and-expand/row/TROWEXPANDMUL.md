...
`TROWEXPANDMUL` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDMUL` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDMUL` |
| ISA 操作 | `TROWEXPANDMUL` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `71` (`0x47`) |
| Function | `7` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8551` |
...
`TROWEXPANDMUL` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDMUL`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDMUL` | `TEPL` | `71` (`0x47`) | `7` | `2` |

`TROWEXPANDMUL` 是 selector-encoded block operation，不是独立 standalone opcode。
