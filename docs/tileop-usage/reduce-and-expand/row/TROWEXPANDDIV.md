...
`TROWEXPANDDIV` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDDIV` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDDIV` |
| ISA 操作 | `TROWEXPANDDIV` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `72` (`0x48`) |
| Function | `8` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8583` |
...
`TROWEXPANDDIV` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDDIV`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDDIV` | `TEPL` | `72` (`0x48`) | `8` | `2` |

`TROWEXPANDDIV` 是 selector-encoded block operation，不是独立 standalone opcode。
