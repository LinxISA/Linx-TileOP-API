...
`TROWEXPANDEXPDIF` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPANDEXPDIF` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TROWEXPANDEXPDIF` |
| ISA 操作 | `TROWEXPANDEXPDIF` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `75` (`0x4b`) |
| Function | `11` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8673` |

## C++ 接口

```cpp
void TROWEXPANDEXPDIF(...);
```
...
`TROWEXPANDEXPDIF` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPANDEXPDIF`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPANDEXPDIF` | `TEPL` | `75` (`0x4b`) | `11` | `2` |

`TROWEXPANDEXPDIF` 是 selector-encoded block operation，不是独立 standalone opcode。
