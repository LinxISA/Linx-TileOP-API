...
`TROWMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWMAX` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `65` (`0x41`) |
| Function | `1` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8228` |
...
`TROWMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TROWMAX` | `TEPL` | `65` (`0x41`) | `1` | `2` |

`TROWMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
