...
`TROWEXPAND` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWEXPAND` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `68` (`0x44`) |
| Function | `4` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8288` |
...
`TROWEXPAND` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWEXPAND`
...
| --- | --- | ---: | ---: | ---: |
| `TROWEXPAND` | `TEPL` | `68` (`0x44`) | `4` | `2` |

`TROWEXPAND` 是 selector-encoded block operation，不是独立 standalone opcode。
