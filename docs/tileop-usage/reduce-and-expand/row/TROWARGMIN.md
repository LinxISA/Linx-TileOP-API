...
`TROWARGMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWARGMIN` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `77` (`0x4d`) |
| Function | `13` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8328` |
...
`TROWARGMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWARGMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TROWARGMIN` | `TEPL` | `77` (`0x4d`) | `13` | `2` |

`TROWARGMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
