...
`TROWPROD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TROWPROD` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `67` (`0x43`) |
| Function | `3` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8268` |
...
`TROWPROD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TROWPROD`
...
| --- | --- | ---: | ---: | ---: |
| `TROWPROD` | `TEPL` | `67` (`0x43`) | `3` | `2` |

`TROWPROD` 是 selector-encoded block operation，不是独立 standalone opcode。
