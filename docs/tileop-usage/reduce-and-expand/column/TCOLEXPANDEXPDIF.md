...
`TCOLEXPANDEXPDIF` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDEXPDIF` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDEXPDIF` |
| ISA 操作 | `TCOLEXPANDEXPDIF` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `91` (`0x5b`) |
| Function | `27` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8883` |

## C++ 接口

```cpp
void TCOLEXPANDEXPDIF(...);
```
...
`TCOLEXPANDEXPDIF` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDEXPDIF`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDEXPDIF` | `TEPL` | `91` (`0x5b`) | `27` | `2` |

`TCOLEXPANDEXPDIF` 是 selector-encoded block operation，不是独立 standalone opcode。
