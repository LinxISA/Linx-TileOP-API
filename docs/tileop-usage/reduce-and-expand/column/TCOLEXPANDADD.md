...
`TCOLEXPANDADD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDADD` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDADD` |
| ISA 操作 | `TCOLEXPANDADD` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `85` (`0x55`) |
| Function | `21` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8703` |
...
`TCOLEXPANDADD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDADD`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDADD` | `TEPL` | `85` (`0x55`) | `21` | `2` |

`TCOLEXPANDADD` 是 selector-encoded block operation，不是独立 standalone opcode。
