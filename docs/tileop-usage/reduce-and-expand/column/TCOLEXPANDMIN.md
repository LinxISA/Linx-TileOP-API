...
`TCOLEXPANDMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDMIN` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDMIN` |
| ISA 操作 | `TCOLEXPANDMIN` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `90` (`0x5a`) |
| Function | `26` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8853` |
...
`TCOLEXPANDMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDMIN` | `TEPL` | `90` (`0x5a`) | `26` | `2` |

`TCOLEXPANDMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
