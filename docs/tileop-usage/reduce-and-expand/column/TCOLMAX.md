...
`TCOLMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLMAX` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `81` (`0x51`) |
| Function | `17` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8368` |
...
`TCOLMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLMAX` | `TEPL` | `81` (`0x51`) | `17` | `2` |

`TCOLMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
