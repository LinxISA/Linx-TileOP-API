...
`TCOLSUM` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLSUM` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `80` (`0x50`) |
| Function | `16` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8348` |
...
`TCOLSUM` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLSUM`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLSUM` | `TEPL` | `80` (`0x50`) | `16` | `2` |

`TCOLSUM` 是 selector-encoded block operation，不是独立 standalone opcode。
