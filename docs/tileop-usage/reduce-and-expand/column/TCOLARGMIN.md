...
`TCOLARGMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLARGMIN` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `93` (`0x5d`) |
| Function | `29` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8468` |
...
`TCOLARGMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLARGMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLARGMIN` | `TEPL` | `93` (`0x5d`) | `29` | `2` |

`TCOLARGMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
