...
`TCOLEXPANDSUB` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDSUB` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDSUB` |
| ISA 操作 | `TCOLEXPANDSUB` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `86` (`0x56`) |
| Function | `22` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8733` |
...
`TCOLEXPANDSUB` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDSUB`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDSUB` | `TEPL` | `86` (`0x56`) | `22` | `2` |

`TCOLEXPANDSUB` 是 selector-encoded block operation，不是独立 standalone opcode。
