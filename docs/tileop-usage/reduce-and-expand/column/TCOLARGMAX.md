...
`TCOLARGMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLARGMAX` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `92` (`0x5c`) |
| Function | `28` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8448` |
...
`TCOLARGMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLARGMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLARGMAX` | `TEPL` | `92` (`0x5c`) | `28` | `2` |

`TCOLARGMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
