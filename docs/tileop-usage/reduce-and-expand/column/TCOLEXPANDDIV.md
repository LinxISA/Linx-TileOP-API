...
`TCOLEXPANDDIV` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDDIV` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDDIV` |
| ISA 操作 | `TCOLEXPANDDIV` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `88` (`0x58`) |
| Function | `24` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8793` |
...
`TCOLEXPANDDIV` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDDIV`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDDIV` | `TEPL` | `88` (`0x58`) | `24` | `2` |

`TCOLEXPANDDIV` 是 selector-encoded block operation，不是独立 standalone opcode。
