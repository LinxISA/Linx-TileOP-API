...
`TCOLPROD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLPROD` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `83` (`0x53`) |
| Function | `19` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8408` |
...
`TCOLPROD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLPROD`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLPROD` | `TEPL` | `83` (`0x53`) | `19` | `2` |

`TCOLPROD` 是 selector-encoded block operation，不是独立 standalone opcode。
