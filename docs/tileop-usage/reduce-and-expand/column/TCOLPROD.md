...
TCOLPROD is a selector-encoded Tile operation executed by SFU. It folds each column in increasing row order with typed multiplication; its current instruction contract owns the exact bundle form and publication boundary.
...
成功调用后，`TCOLPROD` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
using OutputTile = Tile<Location::Vec, float, 32, 32, BLayout::RowMajor, 1, 32>;
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TCOLPROD.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/reduce-and-expand/column-reduction/TCOLPROD.md)。
