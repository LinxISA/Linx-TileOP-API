...
`TCOLEXPANDMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDMAX` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDMAX` |
| ISA 操作 | `TCOLEXPANDMAX` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `89` (`0x59`) |
| Function | `25` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8823` |
...
`TCOLEXPANDMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDMAX` | `TEPL` | `89` (`0x59`) | `25` | `2` |

`TCOLEXPANDMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
