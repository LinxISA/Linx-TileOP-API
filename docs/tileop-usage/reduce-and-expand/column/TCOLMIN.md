...
`TCOLMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLMIN` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `82` (`0x52`) |
| Function | `18` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8388` |
...
`TCOLMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLMIN` | `TEPL` | `82` (`0x52`) | `18` | `2` |

`TCOLMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
