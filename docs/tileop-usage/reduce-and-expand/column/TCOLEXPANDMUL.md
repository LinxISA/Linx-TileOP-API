...
`TCOLEXPANDMUL` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPANDMUL` operation.

## 接口身份

| 项目 | 内容 |
| --- | --- |
| API 名称 | `TCOLEXPANDMUL` |
| ISA 操作 | `TCOLEXPANDMUL` |
| 执行引擎 | `SFU` |
| 指令分类 | `reduce-and-expand` |
| 编码 carrier | `TEPL` |
| Logical selector | `87` (`0x57`) |
| Function | `23` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8763` |
...
`TCOLEXPANDMUL` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPANDMUL`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPANDMUL` | `TEPL` | `87` (`0x57`) | `23` | `2` |

`TCOLEXPANDMUL` 是 selector-encoded block operation，不是独立 standalone opcode。
