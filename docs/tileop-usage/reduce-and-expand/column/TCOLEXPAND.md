...
`TCOLEXPAND` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCOLEXPAND` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `84` (`0x54`) |
| Function | `20` |
| Mode | `2` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8428` |
...
`TCOLEXPAND` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCOLEXPAND`
...
| --- | --- | ---: | ---: | ---: |
| `TCOLEXPAND` | `TEPL` | `84` (`0x54`) | `20` | `2` |

`TCOLEXPAND` 是 selector-encoded block operation，不是独立 standalone opcode。
