...
`TFILLPAD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TFILLPAD` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `101` (`0x65`) |
| Function | `5` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7619` |
...
`TFILLPAD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TFILLPAD`
...
| --- | --- | ---: | ---: | ---: |
| `TFILLPAD` | `TEPL` | `101` (`0x65`) | `5` | `3` |

`TFILLPAD` 是 selector-encoded block operation，不是独立 standalone opcode。
