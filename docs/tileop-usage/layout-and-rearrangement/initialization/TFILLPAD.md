...
成功调用后，`TFILLPAD` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
using namespace pto;
using GM = global_tensor<int32_t, RowMajor<16, 16>>;
using InputTile = Tile<Location::Vec, int32_t, 16, 16, BLayout::RowMajor, 9, 9>;
using OutputTile = Tile<Location::Vec, int32_t, 16, 16, BLayout::RowMajor,
                        16, 16, SLayout::NoneBox, 512, PadValue::Zero>;
int32_t data[16 * 16] = {};
GM src_global(data), dst_global(data);
InputTile src;
OutputTile dst;
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TFILLPAD.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/layout-and-rearrangement/initialization/TFILLPAD.md)。
