...
`TCONCAT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCONCAT` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `96` (`0x60`) |
| Function | `0` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8917` |
...
`TCONCAT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCONCAT`
...
| --- | --- | ---: | ---: | ---: |
| `TCONCAT` | `TEPL` | `96` (`0x60`) | `0` | `3` |

`TCONCAT` 是 selector-encoded block operation，不是独立 standalone opcode。
