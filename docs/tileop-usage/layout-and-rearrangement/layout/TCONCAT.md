...
成功调用后，`TCONCAT` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
using namespace pto;
using GM0 = global_tensor<float, RowMajor<16, 8>>;
using GM1 = global_tensor<float, RowMajor<16, 8>>;
using GMO = global_tensor<float, RowMajor<16, 16>>;
using Tile0 = Tile<Location::Vec, float, 16, 8, BLayout::RowMajor, 16, 8>;
using TileO = Tile<Location::Vec, float, 16, 16, BLayout::RowMajor, 16, 16>;
float a[16 * 8] = {}, b[16 * 8] = {}, out[16 * 16] = {};
GM0 g0(a); GM1 g1(b); GMO go(out);
Tile0 src0, src1; TileO dst;
TLOAD(src0, g0); TLOAD(src1, g1);
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TCONCAT.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/layout-and-rearrangement/layout/TCONCAT.md)。
