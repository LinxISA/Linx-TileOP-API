...
`TINSERT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TINSERT` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `99` (`0x63`) |
| Function | `3` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7561` |
...
`TINSERT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TINSERT`
...
| --- | --- | ---: | ---: | ---: |
| `TINSERT` | `TEPL` | `99` (`0x63`) | `3` | `3` |

`TINSERT` 是 selector-encoded block operation，不是独立 standalone opcode。
