...
| `control` | 低两个 control byte 分别给出 `src0`、`src1` 字段宽度：`control[7:0]` 和 `control[15:8]`。运行时会拒绝高 32 位非零、宽度不在 `1..3` 或宽度总和超过 4 的 control。 |

## 约束

- 所有 Tile 必须是 Local U32，使用相同的 `CUBE_M16` 或 `CUBE_M32` layout 和几何。
- `control[7:0]` 是 `src0` 字段宽度，`control[15:8]` 是 `src1` 字段宽度；两者都必须在 `1..3` bytes，且宽度之和不能超过 4 bytes。
