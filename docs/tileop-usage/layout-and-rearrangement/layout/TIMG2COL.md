...
`TIMG2COL` is a TileOP C++ interface for the PTO ISA v0.58.3 `TIMG2COL` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| ISA 操作 | `TIMG2COL` |
| 执行引擎 | `SFU` |
| 指令分类 | `layout-and-rearrangement` |
| 编码 carrier | `TEPL` |
| Logical selector | `100` (`0x64`) |
| Function | `4` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7593` |
...
`TIMG2COL` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TIMG2COL`
...
| --- | --- | ---: | ---: | ---: |
| `TIMG2COL` | `TEPL` | `100` (`0x64`) | `4` | `3` |

`TIMG2COL` 是 selector-encoded block operation，不是独立 standalone opcode。
