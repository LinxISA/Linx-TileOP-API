...
`TTRANS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TTRANS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `110` (`0x6e`) |
| Function | `14` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8056` |
...
`TTRANS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TTRANS`
...
| --- | --- | ---: | ---: | ---: |
| `TTRANS` | `TEPL` | `110` (`0x6e`) | `14` | `3` |

`TTRANS` 是 selector-encoded block operation，不是独立 standalone opcode。
