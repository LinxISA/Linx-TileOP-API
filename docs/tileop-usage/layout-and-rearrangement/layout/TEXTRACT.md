...
成功调用后，`TEXTRACT` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
using namespace pto;
using InGM = global_tensor<float, RowMajor<32, 16>>;
using OutGM = global_tensor<float, RowMajor<16, 8>>;
using InTile = Tile<Location::Vec, float, 32, 16, BLayout::RowMajor>;
using OutTile = Tile<Location::Vec, float, 16, 8, BLayout::RowMajor>;
float src_data[32 * 16] = {}, dst_data[16 * 8] = {};
InGM src_global(src_data); OutGM dst_global(dst_data);
InTile src; OutTile dst;
TLOAD(src, src_global);
TEXTRACT(dst, src, 16, 8);
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TEXTRACT.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/layout-and-rearrangement/layout/TEXTRACT.md)。
