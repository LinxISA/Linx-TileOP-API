...
`TEXTRACT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TEXTRACT` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `98` (`0x62`) |
| Function | `2` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7534` |
...
`TEXTRACT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TEXTRACT`
...
| --- | --- | ---: | ---: | ---: |
| `TEXTRACT` | `TEPL` | `98` (`0x62`) | `2` | `3` |

`TEXTRACT` 是 selector-encoded block operation，不是独立 standalone opcode。
