...
`TEXPANDS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TEXPANDS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `59` (`0x3b`) |
| Function | `27` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7482` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TEXPANDS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TEXPANDS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TEXPANDS`
...
| --- | --- | ---: | ---: | ---: |
| `TEXPANDS` | `TEPL` | `59` (`0x3b`) | `27` | `1` |

`TEXPANDS` 是 selector-encoded block operation，不是独立 standalone opcode。
