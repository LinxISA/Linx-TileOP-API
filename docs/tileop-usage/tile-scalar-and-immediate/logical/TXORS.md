...
`TXORS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TXORS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `40` (`0x28`) |
| Function | `8` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7161` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TXORS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TXORS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TXORS`
...
| --- | --- | ---: | ---: | ---: |
| `TXORS` | `TEPL` | `40` (`0x28`) | `8` | `1` |

`TXORS` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.VEC TXORS, DataType
