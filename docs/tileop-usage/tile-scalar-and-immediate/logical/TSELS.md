...
`TSELS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSELS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `58` (`0x3a`) |
| Function | `26` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7456` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TSELS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TSELS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSELS`
...
| --- | --- | ---: | ---: | ---: |
| `TSELS` | `TEPL` | `58` (`0x3a`) | `26` | `1` |

`TSELS` 是 selector-encoded block operation，不是独立 standalone opcode。
