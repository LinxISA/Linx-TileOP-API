...
TSHRS right-shifts every valid integer element by one scalar count and publishes a new Local destination.
...
成功调用后，`TSHRS` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TSHRS.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/tile-scalar-and-immediate/logical/TSHRS.md)。
