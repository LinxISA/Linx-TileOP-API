...
`TSHRS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSHRS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `42` (`0x2a`) |
| Function | `10` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7211` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TSHRS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TSHRS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSHRS`
...
| --- | --- | ---: | ---: | ---: |
| `TSHRS` | `TEPL` | `42` (`0x2a`) | `10` | `1` |

`TSHRS` 是 selector-encoded block operation，不是独立 standalone opcode。
