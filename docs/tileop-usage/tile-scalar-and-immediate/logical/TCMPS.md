...
成功调用后，`TCMPS` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
int32_t dst_data[64 * 32] = {};
InputGM src_global(src_data);
ResultGM dst_global(dst_data);
InputTile src;
ResultTile dst;
InputTile::DType scalar = 0.0f;
TLOAD(src, src_global);
TCMPS<CmpMode::EQ>(dst, src, scalar);
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TCMPS.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/tile-scalar-and-immediate/logical/TCMPS.md)。
