...
`TCMPS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCMPS` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| ISA 操作 | `TCMPS` |
| 执行引擎 | `VEC` |
| 指令分类 | `tile-scalar-and-immediate` |
| 编码 carrier | `TEPL` |
| Logical selector | `45` (`0x2d`) |
| Function | `13` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7289` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TCMPS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TCMPS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCMPS`
...
| --- | --- | ---: | ---: | ---: |
| `TCMPS` | `TEPL` | `45` (`0x2d`) | `13` | `1` |

`TCMPS` 是 selector-encoded block operation，不是独立 standalone opcode。
