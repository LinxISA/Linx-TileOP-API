...
`TANDS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TANDS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `38` (`0x26`) |
| Function | `6` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7111` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TANDS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TANDS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TANDS`
...
| --- | --- | ---: | ---: | ---: |
| `TANDS` | `TEPL` | `38` (`0x26`) | `6` | `1` |

`TANDS` 是 selector-encoded block operation，不是独立 standalone opcode。
