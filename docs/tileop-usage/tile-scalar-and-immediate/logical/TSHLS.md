...
`TSHLS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSHLS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `41` (`0x29`) |
| Function | `9` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7186` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TSHLS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TSHLS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSHLS`
...
| --- | --- | ---: | ---: | ---: |
| `TSHLS` | `TEPL` | `41` (`0x29`) | `9` | `1` |

`TSHLS` 是 selector-encoded block operation，不是独立 standalone opcode。
