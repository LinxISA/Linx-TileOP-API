...
`TORS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TORS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `39` (`0x27`) |
| Function | `7` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7136` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TORS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TORS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TORS`
...
| --- | --- | ---: | ---: | ---: |
| `TORS` | `TEPL` | `39` (`0x27`) | `7` | `1` |

`TORS` 是 selector-encoded block operation，不是独立 standalone opcode。
