...
`TMULS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMULS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `34` (`0x22`) |
| Function | `2` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7026` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TMULS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TMULS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMULS`
...
| --- | --- | ---: | ---: | ---: |
| `TMULS` | `TEPL` | `34` (`0x22`) | `2` | `1` |

`TMULS` 是 selector-encoded block operation，不是独立 standalone opcode。
