...
`TMAXS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMAXS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `43` (`0x2b`) |
| Function | `11` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7236` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TMAXS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TMAXS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMAXS`
...
| --- | --- | ---: | ---: | ---: |
| `TMAXS` | `TEPL` | `43` (`0x2b`) | `11` | `1` |

`TMAXS` 是 selector-encoded block operation，不是独立 standalone opcode。
