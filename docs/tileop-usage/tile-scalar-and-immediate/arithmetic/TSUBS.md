...
`TSUBS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSUBS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `33` (`0x21`) |
| Function | `1` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7001` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TSUBS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TSUBS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSUBS`
...
| --- | --- | ---: | ---: | ---: |
| `TSUBS` | `TEPL` | `33` (`0x21`) | `1` | `1` |

`TSUBS` 是 selector-encoded block operation，不是独立 standalone opcode。
