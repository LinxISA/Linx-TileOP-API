...
`TREMS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TREMS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `36` (`0x24`) |
| Function | `4` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7078` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TREMS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TREMS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TREMS`
...
| --- | --- | ---: | ---: | ---: |
| `TREMS` | `TEPL` | `36` (`0x24`) | `4` | `1` |

`TREMS` 是 selector-encoded block operation，不是独立 standalone opcode。
