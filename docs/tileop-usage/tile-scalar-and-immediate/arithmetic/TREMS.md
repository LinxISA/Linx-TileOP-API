...
TREMS computes divisor-signed modulo between each valid element and one scalar and publishes a new Local destination.
...
成功调用后，`TREMS` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
float src_data[64 * 32] = {}, dst_data[64 * 32] = {};
GM src_global(src_data), dst_global(dst_data);
TileT src, dst;
TileT::DType scalar = 2.0f;
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TREMS.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/tile-scalar-and-immediate/arithmetic/TREMS.md)。
