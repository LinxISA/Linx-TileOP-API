...
`TMINS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMINS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `44` (`0x2c`) |
| Function | `12` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7261` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TMINS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TMINS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMINS`
...
| --- | --- | ---: | ---: | ---: |
| `TMINS` | `TEPL` | `44` (`0x2c`) | `12` | `1` |

`TMINS` 是 selector-encoded block operation，不是独立 standalone opcode。
