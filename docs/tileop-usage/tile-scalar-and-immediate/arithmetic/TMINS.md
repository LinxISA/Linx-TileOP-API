...
TMINS selects the typed minimum of each valid Tile element and one scalar.
...
成功调用后，`TMINS` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TMINS.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/tile-scalar-and-immediate/arithmetic/TMINS.md)。
