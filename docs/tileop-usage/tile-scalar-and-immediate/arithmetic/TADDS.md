...
`TADDS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TADDS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `32` (`0x20`) |
| Function | `0` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6974` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TADDS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TADDS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TADDS`
...
| --- | --- | ---: | ---: | ---: |
| `TADDS` | `TEPL` | `32` (`0x20`) | `0` | `1` |

`TADDS` 是 selector-encoded block operation，不是独立 standalone opcode。
