...
`TDIVS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TDIVS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `35` (`0x23`) |
| Function | `3` |
| Mode | `1` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7053` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src>
void TDIVS(Dst &dst, Src &src, typename Src::DType scalar);
```
...
`TDIVS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TDIVS`
...
| --- | --- | ---: | ---: | ---: |
| `TDIVS` | `TEPL` | `35` (`0x23`) | `3` | `1` |

`TDIVS` 是 selector-encoded block operation，不是独立 standalone opcode。
