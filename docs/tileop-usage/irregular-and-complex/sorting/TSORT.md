...
`TSORT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSORT` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 编码 carrier | `TEPL` |
| Logical selector | `108` (`0x6c`) |
| Function | `12` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7923` |

## C++ 接口

```cpp
void TSORT(...);  // see sort.md for the complete typed signature
```
...
`TSORT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSORT`
...
| --- | --- | ---: | ---: | ---: |
| `TSORT` | `TEPL` | `108` (`0x6c`) | `12` | `3` |

`TSORT` 是 selector-encoded block operation，不是独立 standalone opcode。
