...
`TMRGSORT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TMRGSORT` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 编码 carrier | `TEPL` |
| Logical selector | `109` (`0x6d`) |
| Function | `13` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8005` |

## C++ 接口

```cpp
void TMRGSORT(...);  // see sort.md for the complete typed signature
```
...
`TMRGSORT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TMRGSORT`
...
| --- | --- | ---: | ---: | ---: |
| `TMRGSORT` | `TEPL` | `109` (`0x6d`) | `13` | `3` |

`TMRGSORT` 是 selector-encoded block operation，不是独立 standalone opcode。
