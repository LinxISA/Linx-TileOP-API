...
`TDEQUANT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TDEQUANT` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 编码 carrier | `TEPL` |
| Logical selector | `107` (`0x6b`) |
| Function | `11` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7828` |

## C++ 接口

```cpp
template <RoundMode Mode, is_tile_data_v Dst, is_tile_data_v Src>
void TDEQUANT(Dst &dst, Src &src, ...);
```
...
`TDEQUANT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TDEQUANT`
...
| --- | --- | ---: | ---: | ---: |
| `TDEQUANT` | `TEPL` | `107` (`0x6b`) | `11` | `3` |

`TDEQUANT` 是 selector-encoded block operation，不是独立 standalone opcode。
