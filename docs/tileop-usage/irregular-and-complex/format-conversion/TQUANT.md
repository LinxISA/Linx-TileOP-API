...
`TQUANT` is a TileOP C++ interface for the PTO ISA v0.58.3 `TQUANT` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 编码 carrier | `TEPL` |
| Logical selector | `106` (`0x6a`) |
| Function | `10` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7713` |

## C++ 接口

```cpp
template <RoundMode Mode, is_tile_data_v Dst, is_tile_data_v Src>
void TQUANT(Dst &dst, Src &src, ...);
```
...
`TQUANT` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TQUANT`
...
| --- | --- | ---: | ---: | ---: |
| `TQUANT` | `TEPL` | `106` (`0x6a`) | `10` | `3` |

`TQUANT` 是 selector-encoded block operation，不是独立 standalone opcode。
