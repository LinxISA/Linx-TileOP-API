...
`TPARTMIN` is a TileOP C++ interface for the PTO ISA v0.58.3 `TPARTMIN` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `116` (`0x74`) |
| Function | `20` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8183` |
...
`TPARTMIN` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TPARTMIN`
...
| --- | --- | ---: | ---: | ---: |
| `TPARTMIN` | `TEPL` | `116` (`0x74`) | `20` | `3` |

`TPARTMIN` 是 selector-encoded block operation，不是独立 standalone opcode。
