...
`TGATHER` is a TileOP C++ interface for the PTO ISA v0.58.3 `TGATHER` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `111` (`0x6f`) |
| Function | `15` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8076` |
...
`TGATHER` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TGATHER`
...
| --- | --- | ---: | ---: | ---: |
| `TGATHER` | `TEPL` | `111` (`0x6f`) | `15` | `3` |

`TGATHER` 是 selector-encoded block operation，不是独立 standalone opcode。
