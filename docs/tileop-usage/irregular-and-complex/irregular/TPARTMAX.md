...
> **注意**：`TPARTMAX` 已从 PTO ISA 0.58.6 active catalog 移除（deleted_names）。本页保留作历史参考；新代码不应使用该操作。
...
## 结果说明
    ...
    成功调用后，`TPARTMAX` 更新输出 Tile 的有效区域；输入 Tile 通常保持不变，输出 padding 和未明确声明的副作用不可依赖。若操作的约束或参数说明另有规定，以对应说明为准。
