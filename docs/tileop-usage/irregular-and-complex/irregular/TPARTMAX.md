...
`TPARTMAX` is a TileOP C++ interface for the PTO ISA v0.58.3 `TPARTMAX` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `115` (`0x73`) |
| Function | `19` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8162` |
...
`TPARTMAX` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TPARTMAX`
...
| --- | --- | ---: | ---: | ---: |
| `TPARTMAX` | `TEPL` | `115` (`0x73`) | `19` | `3` |

`TPARTMAX` 是 selector-encoded block operation，不是独立 standalone opcode。
