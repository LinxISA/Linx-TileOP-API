...
`TPARTADD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TPARTADD` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `113` (`0x71`) |
| Function | `17` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8120` |
...
`TPARTADD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TPARTADD`
...
| --- | --- | ---: | ---: | ---: |
| `TPARTADD` | `TEPL` | `113` (`0x71`) | `17` | `3` |

`TPARTADD` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.SFU TPARTADD, DataType
