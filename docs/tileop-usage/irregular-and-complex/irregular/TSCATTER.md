...
`TSCATTER` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSCATTER` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `112` (`0x70`) |
| Function | `16` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8098` |
...
`TSCATTER` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSCATTER`
...
| --- | --- | ---: | ---: | ---: |
| `TSCATTER` | `TEPL` | `112` (`0x70`) | `16` | `3` |

`TSCATTER` 是 selector-encoded block operation，不是独立 standalone opcode。

## Bundle composition

```asm
BSTART.SFU TSCATTER, DataType
