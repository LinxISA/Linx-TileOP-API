...
`TPARTMUL` is a TileOP C++ interface for the PTO ISA v0.58.3 `TPARTMUL` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `114` (`0x72`) |
| Function | `18` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:8141` |
...
`TPARTMUL` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TPARTMUL`
...
| --- | --- | ---: | ---: | ---: |
| `TPARTMUL` | `TEPL` | `114` (`0x72`) | `18` | `3` |

`TPARTMUL` 是 selector-encoded block operation，不是独立 standalone opcode。
