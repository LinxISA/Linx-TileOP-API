...
> **注意**：`TPARTMUL` 已从当前 PTO ISA catalog（main `961fa81e`）的活跃操作集中移除（deleted_names）。本页保留作历史参考；新代码不应使用该操作。

`TPARTMUL` 构造以原点对齐的并集，并将重叠元素相乘。
...
## 结果说明
    ...
    成功调用后，`TPARTMUL` 更新输出 Tile 的有效区域；输入 Tile 通常保持不变，输出 padding 和未明确声明的副作用不可依赖。若操作的约束或参数说明另有规定，以对应说明为准。
