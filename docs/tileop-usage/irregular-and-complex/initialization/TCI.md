...
`TCI` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCI` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 编码 carrier | `TEPL` |
| Logical selector | `102` (`0x66`) |
| Function | `6` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7639` |
...
`TCI` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCI`
...
| --- | --- | ---: | ---: | ---: |
| `TCI` | `TEPL` | `102` (`0x66`) | `6` | `3` |

`TCI` 是 selector-encoded block operation，不是独立 standalone opcode。
