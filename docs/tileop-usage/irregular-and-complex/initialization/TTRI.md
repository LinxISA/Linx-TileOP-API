...
`TTRI` is a TileOP C++ interface for the PTO ISA v0.58.3 `TTRI` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `103` (`0x67`) |
| Function | `7` |
| Mode | `3` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7680` |
...
`TTRI` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TTRI`
...
| --- | --- | ---: | ---: | ---: |
| `TTRI` | `TEPL` | `103` (`0x67`) | `7` | `3` |

`TTRI` 是 selector-encoded block operation，不是独立 standalone opcode。
