...
`TSUB` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSUB` operation.
...
| Function | `1` |
| Mode | `0` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6370` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src0, is_tile_data_v Src1>
void TSUB(Dst &dst, Src0 &src0, Src1 &src1);
```
...
`TSUB` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSUB`
...
| --- | --- | ---: | ---: | ---: |
| `TSUB` | `TEPL` | `1` (`0x01`) | `1` | `0` |

`TSUB` 是 selector-encoded block operation，不是独立 standalone opcode。
