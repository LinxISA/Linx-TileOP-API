...
`TADD` is a TileOP C++ interface for the PTO ISA v0.58.3 `TADD` operation.
...
| Function | `0` |
| Mode | `0` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6349` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src0, is_tile_data_v Src1>
void TADD(Dst &dst, Src0 &src0, Src1 &src1);
```
...
`TADD` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TADD`
...
| --- | --- | ---: | ---: | ---: |
| `TADD` | `TEPL` | `0` (`0x00`) | `0` | `0` |

`TADD` 是 selector-encoded block operation，不是独立 standalone opcode。
