...
成功调用后，`TFMA` 按操作语义更新输出 Tile。padding、输入持久性、边界行为及数值状态影响请以 PTO-SPEC 为准；未明确声明的副作用不应被假定。
...
float a_data[64 * 32] = {}, b_data[64 * 32] = {}, c_data[64 * 32] = {}, result_data[64 * 32] = {};
GM a_global(a_data), b_global(b_data), c_global(c_data), result_global(result_data);
TileT a, b, c, result;
TLOAD(a, a_global);
TLOAD(b, b_global);
TLOAD(c, c_global);
...
完整语义、约束、默认值、异常和边界行为请参阅 [`TFMA.md`](https://github.com/PTO-ISA/pto-spec/blob/v0.58.4.1/docs/tile/elementwise-tile-tile/arithmetic/TFMA.md)。
