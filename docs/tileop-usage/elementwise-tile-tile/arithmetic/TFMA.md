...
`TFMA` is a TileOP C++ interface for the PTO ISA v0.58.3 `TFMA` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `28` (`0x1c`) |
| Function | `28` |
| Mode | `0` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:7509` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src0, is_tile_data_v Src1>
void TFMA(Dst &dst, Src0 &src0, Src1 &src1);
```
...
`TFMA` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TFMA`
...
| --- | --- | ---: | ---: | ---: |
| `TFMA` | `TEPL` | `28` (`0x1c`) | `28` | `0` |

`TFMA` 是 selector-encoded block operation，不是独立 standalone opcode。
