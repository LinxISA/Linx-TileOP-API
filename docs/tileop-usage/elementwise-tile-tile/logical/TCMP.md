...
`TCMP` is a TileOP C++ interface for the PTO ISA v0.58.3 `TCMP` operation. See the [existing topic guide](../../concepts/README.md) for compatibility and operation-specific examples.
...
| 编码 carrier | `TEPL` |
| Logical selector | `13` (`0x0d`) |
| Function | `13` |
| Mode | `0` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6614` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src0, is_tile_data_v Src1>
void TCMP(Dst &dst, Src0 &src0, Src1 &src1);
```
...
`TCMP` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TCMP`
...
| --- | --- | ---: | ---: | ---: |
| `TCMP` | `TEPL` | `13` (`0x0d`) | `13` | `0` |

`TCMP` 是 selector-encoded block operation，不是独立 standalone opcode。
