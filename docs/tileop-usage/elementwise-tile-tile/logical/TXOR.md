...
`TXOR` 对对应整数元素执行按位异或，并并发布新的 Local 目标。
...
### Associated destination：`TXOR_ASS`

```cpp
template <is_tile_data_v D, is_tile_data_v A, is_tile_data_v B>
void TXOR_ASS(D &assembled_dst, A &src0, B &src1);
```
...
## 结果说明
    ...
    成功调用后，`TXOR` 更新输出 Tile 的有效区域；输入 Tile 通常保持不变，输出 padding 和未明确声明的副作用不可依赖。若操作的约束或参数说明另有规定，以对应说明为准。
