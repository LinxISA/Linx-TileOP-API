...
`TSEL` is a TileOP C++ interface for the PTO ISA v0.58.3 `TSEL` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `26` (`0x1a`) |
| Function | `26` |
| Mode | `0` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6746` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src0, is_tile_data_v Src1>
void TSEL(Dst &dst, Src0 &src0, Src1 &src1);
```
...
`TSEL` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TSEL`
...
| --- | --- | ---: | ---: | ---: |
| `TSEL` | `TEPL` | `26` (`0x1a`) | `26` | `0` |

`TSEL` 是 selector-encoded block operation，不是独立 standalone opcode。
