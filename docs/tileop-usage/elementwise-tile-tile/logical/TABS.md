...
`TABS` is a TileOP C++ interface for the PTO ISA v0.58.3 `TABS` operation.
...
| 编码 carrier | `TEPL` |
| Logical selector | `15` (`0x0f`) |
| Function | `15` |
| Mode | `0` |
| C++ 定义位置 | `include/jcore/template_asm.hpp:6767` |

## C++ 接口

```cpp
template <is_tile_data_v Dst, is_tile_data_v Src0, is_tile_data_v Src1>
void TABS(Dst &dst, Src0 &src0, Src1 &src1);
```
...
`TABS` 在所有合法操作数和 Tile 描述符完成检查后，对有效区域执行 `TABS`
...
| --- | --- | ---: | ---: | ---: |
| `TABS` | `TEPL` | `15` (`0x0f`) | `15` | `0` |

`TABS` 是 selector-encoded block operation，不是独立 standalone opcode。
