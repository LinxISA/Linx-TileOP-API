...
class LinxISAV058EngineContractTest(unittest.TestCase):
    ...
    def test_range_modifiers_expose_simple_factories(self) -> None:
        ...
        self.assertIn("auto sv = range::subview(s, base);", tile)
        self.assertIn("auto sv = range::subview<128, 2047>(s);", tile)
        self.assertIn("auto sv = range::subview<128, 2047>(s, base);", tile)
        self.assertIn("auto sv = range::subview_at_reg<2047, 23>(s, 23);", tile)
        ...
        self.assertIn("LengthBytes_ = 0, unsigned Offset_ = 0", tile_header)
