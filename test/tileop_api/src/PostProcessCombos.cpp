...
void tmatmul_combos(D &d, Ds8 &d8, Df16 &df16, D &c, A &a, B &b, P &q, P &p,
  ...
  TMATMUL_MX(d, a, q, b, q, fixp::keep_acc().group_max<16>(gout));
  TMATMUL_MX_ACC(d8, c, a, q, b, q, fixp::s8(s8_desc));
  TMATMUL_MX_BIAS(d8, a, q, b, q, c, fixp::s8(q).prelu(p));
}

// --- TGEMV family ---
void tgemv_combos(GV_D &d, GV_Ds8 &d8, GV_Df16 &df16, GV_D &c, GV_V &v, GV_M &m,
  ...
  TGEMV_MX(d, m, q, v, q, fixp::keep_acc().group_max<16>(gout));
  TGEMV_MX_ACC(d8, c, m, q, v, q, fixp::s8(s8_desc));
  TGEMV_MX_BIAS(d8, m, q, v, q, c, fixp::s8(q).prelu(p));
}
