...
void signed_matrix(SD &d, SQ &q, SD &c, SA &a, SB &b, SBias &bias,
                   SRow &row) {
  TMATMUL(d, a, b);
  TMATMUL_ACC(d, c, a, b);
  TMATMUL_BIAS(d, a, b, bias);
  TMATMUL(d, a, b, fixp::keep_acc().row_max(row));
  TMATMUL(q, a, b, fixp::s8(0));
}
